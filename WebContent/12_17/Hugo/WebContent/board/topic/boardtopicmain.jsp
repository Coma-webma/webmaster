...
<link href="../../css/boardtopicmain.css" rel="stylesheet"
	type="text/css">
			...
			<nav>
				...
				<nav id="main_bar">
					<ul class="main_bar_all">
						...
						%>
						<a
							href="../module/boardmodulemodify.jsp?m_idx=<%=rs.getString("m_idx")%>">
							<li>▶ <%=m_title%></li>
						</a>
						<%
							}
		...
		<article>
				...
				<ul>
					<li><input type="radio" id="function_basic" name="function"
						value> <label for="function_basic"><a
							href="../module/boardmodulemodify.jsp" style="color: black">커버</a></label></li>
					<li><label for="function_topic"><a href="boardtopicmain.jsp" style="color: black">토픽목록</a></label></li>
				</ul>
			</fieldset>
			<%
				rs.beforeFirst();
			...
			<section>
				<div class="select">
					...
					<div class="make">
						<a href="boardtopic.jsp?m_idx=<%=m_idx%>" style="color: white">토픽만들기</a>
						<!-- ../topic/boardtopicmain.jsp?m_idx=<%=m_idx%> -->
					</div>
				</div>
				<table class="a12">
					<thead>
					...
					<tbody class="border1">
						<%
							...
							try {
								...
								conn = DriverManager.getConnection("jdbc:mariadb://localhost:3306/coma", "root", "1234");
								if (conn != null) {
									sql = "select *from topic where t_midx = ?";
									pstmt = conn.prepareStatement(sql);
						...
						<tr>
							...
							<td><a
								href="boardtopicmodify.jsp?t_idx=<%=rs2.getString("t_idx")%>"><strong><%=t_title%></strong></a></td>
							<td class="public1">●</td>
							<td class="public1">●</td>
							<td class="date"><%=t_date%></td>
							<td class="del"><a
								href="../boardDell.jsp?idx=<%=rs2.getString("t_idx")%>&idx_dell=topic">삭제</a></td>
						</tr>
						<%
							}
								}
